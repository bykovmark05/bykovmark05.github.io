document.getElementById("div1").style.display = "none";
	...
	var trigger=0, name, age, sex, education, mathematic, asu, cutting, programming; // обьявляем переменные которые понадобятся в ходе работы
		...
		do {
			name = prompt("Введите имя (только буквы)");
			} while (!/^[а-яА-ЯёЁa-zA-Z]+$/.test(name));
		do {
			age = prompt("Введите возраст (целое число > 0)");
			} while (!/^\d+$/.test(age) || parseInt(age) <= 0);
		do {
			sex = prompt("Введите пол (М или Ж)");
			} while (sex !== "М" && sex !== "Ж");name = prompt("Введите имя");
